...
void Client::DrawHUD() {
	...
	std::string text = tfm::format(XOR("neverm1ss aka plakhov | lat: %ims | rate: %i | %s"), ms, g_cl.m_rate, time.str());
	...
	if (g_cfg[XOR("misc_preserve_killfeed")].get<bool>())
	{
		...
		if (g_cl.m_local->alive()) {
			...
			if (clear_death_notices_addr && death_notices_addr) {
				...
				if (g_cl.m_round_flags == 0 && local_death_notice_time)
			...
			// if( !g_cfg[ XOR( "misc_preserve_killfeed" ) ].get<bool>( ) ) {
				if (local_death_notice_time && *local_death_notice_time != std::numeric_limits<float>::max()) {
				...
				//if( g_cfg[ XOR( "misc_preserve_killfeed" ) ].get<bool>( ) ) {
				if (g_cl.m_round_flags == 1 && death_notices_addr - 0x14) {
					clear_notices = true;
					*local_death_notice_time = backup_local_death_notice_time;
				}
				//}
...
void Client::ClanTag( )
{
	...
	auto SetClanTag = [ & ] ( std::string tag ) -> void {
		...
		SetClanTagFn( tag.c_str( ), XOR( "neverm1ss" ) );
	};

	std::string szClanTag = XOR( "neverm1ss " );
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( clr, XOR( "[ neverm1ss ] " ) );
