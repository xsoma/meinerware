			...
			//ResolveStand(data, record);
		{
			...
			if (speed < 0.1f)
			{
				...
				if (layers->m_cycle > 0.9f && layers->m_weight > 0.9f)
				{
					...
					auto absed_lby_delta = std::abs(math::NormalizeYaw(lby_delta));
					if (absed_lby_delta > 30.f)
					{
						if (lby_delta > 0.f)
						{
							record->m_pState->m_flGoalFeetYaw = math::NormalizeYaw(player->m_angEyeAngles().y + 60.f);
				...
				else
				{
					if (data->m_missed_shots == 0)
						record->m_pState->m_flGoalFeetYaw = math::NormalizeYaw(player->m_angEyeAngles().y + 60.f * last_resolving_side[player->index()]);
					else
					{
