...
void GUI::Form::EndWindow( ) {
	...
	D3D::RectFilledLinearGradient( ctx->pos + vec2_t( 0, 60 ), vec2_t( ctx->size.x, 5 ), Color( 35, 40, 49 ).OverrideAlpha( 255 * ctx->animation, true ), Color::Palette_t::MenuColors_t::Form( ).OverrideAlpha( 255 * ctx->animation, true ) );




	static float rainbow = 40 * g_csgo.m_globals->m_frametime;
	rainbow -= 10;
	if (rainbow < 0)
		rainbow += 255;

	static float rainbow2 = 10 * g_csgo.m_globals->m_frametime;
	rainbow2 -= 10;
	if (rainbow2 < 0)
		rainbow2 += 255;
	...
	// titlebar texture
	//D3D::DrawTexture( GUI::ctx->pos, vec2_t( GUI::ctx->size.x, 60), D3D::TITLEBAR_TEXTURE, ctx->animation );
	D3D::RectFilled(ctx->pos + vec2_t(0, 0), vec2_t(800, 50), Color(210, 40, 49).OverrideAlpha(140 * ctx->animation, true));

	D3D::RectFilled(ctx->pos + vec2_t(0, 620), vec2_t(800, 26), Color(210, 40, 49).OverrideAlpha(140 * ctx->animation, true));

	D3D::RectFilled(ctx->pos + vec2_t(0, 50), vec2_t(2, 570), Color(210, 40, 49).OverrideAlpha(140 * ctx->animation, true));

	D3D::RectFilled(ctx->pos + vec2_t(798, 50), vec2_t(2, 570), Color(210, 40, 49).OverrideAlpha(140 * ctx->animation, true));

	// "pandora" text
	D3D::Text( ctx->pos + vec2_t( 25, 4 ), XOR( "MeinerWare" ), Color( 50, 52, 255).OverrideAlpha(rainbow * ctx->animation, true ), D3D::D3D9Fonts[ D3D::MeinerWare_FONT], 0 );

	D3D::Text( ctx->pos + vec2_t( 35, 4 ), XOR( "MeinerWare" ), Color( 255, 0, 72 ).OverrideAlpha(rainbow2 * ctx->animation, true ), D3D::D3D9Fonts[ D3D::MeinerWare_FONT], 0 );

	D3D::Text(ctx->pos + vec2_t(30, 5), XOR("MeinerWare"), Color(220, 220, 220).OverrideAlpha(255 * ctx->animation, true), D3D::D3D9Fonts[D3D::MeinerWare_FONT], 0);

	D3D::Text( ctx->pos + vec2_t(10, 625), XOR( "Alpha for Counter-Strike: Global Offensive | UID: 0 | Online:2" ), Color( 220, 220, 220 ).OverrideAlpha( 255 * ctx->animation, true ), D3D::D3D9Fonts[ D3D::MAIN_FONT], 0 );
	...
	if( !ctx->Tabs.empty( ) ) {
		...
		for( int i = 0; i < ctx->Tabs.size( ); ++i ) {
			TabPos.at( i ) = ( i == 0 ) ? ctx->pos + vec2_t( ctx->size.x / 3, 10 ) : TabPos.at( i - 1 ) + vec2_t( TabSize.at( i - 1 ).x + 30, 0 );
			...
			D3D::RectFilled(TabPos.at(i) + vec2_t(-15, 40), vec2_t((TabSize.at(i).x + 30) * HoverAnimation.at(i), -50),
				Color::Palette_t::MenuColors_t::Outlines().OverrideAlpha(100 * ctx->animation, true));

			D3D::RectFilled(TabPos.at(i) + vec2_t(-15, 40), vec2_t(((TabSize.at(i).x + 30) * SelectAnimation.at(i)), -50),
				g_cfg[XOR("menu_color")].get_color().OverrideAlpha(100 * ctx->animation, true));	
	...
	if( !ctx->SubTabs.empty( ) && !ctx->SubTabs.at( ctx->ActiveTab ).empty( ) ) {
		...
		for( int i = 0; i < SubTab.size( ); ++i ) {
			//SubTabSize.at( i ) = vec2_t( int( ( ctx->size.x ) / SubTab.size( ) ), 100 ); 
			SubTabSize.at( i ) = vec2_t( 70, (ctx->size.y / SubTab.size() - 45)); 
			SubTabPos.at( i ) = ( i == 0 ) ? ctx->pos + vec2_t(3, 63) : SubTabPos.at( i - 1 ) + vec2_t(0, SubTabSize.at( i - 1 ).y);
